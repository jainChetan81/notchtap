...
- **in**: a second webview window (`settings` label), a fourth tray
  item, four invoke commands, config validation + atomic write +
  relaunch, a secrets write path (openrouter api key now, telegram
  folded in), a persisted `start_paused` kill switch, the `build.rs`
  command-acl opt-in that keeps the overlay receive-only.
- **not in v5**: hot-reload / file-watcher (restart is the reload
  mechanism, locked), any ai feature consuming the openrouter key,
  animation previews in the panel (future — the window is sized so
  they *can* land later), keychain storage, editing `detect_path`
  from the ui, multi-window sync (the panel reads the booted config;
  concurrent hand-edits of the file lose).
...
2. **new capability** `src-tauri/capabilities/settings.json` —
   ...
   `allow-save-config-and-relaunch`, `allow-set-secret`, plus
   `core:event:allow-listen`/`allow-unlisten` (future previews).
...
|---|---|---|
| `get_config` | `() -> Config` | returns the **booted** config (managed `Arc<Config>` state), not a fresh file read — "what is running", which is what save-and-relaunch semantics make true again after every save. `Config` gains `#[derive(Serialize, Clone)]`. |
