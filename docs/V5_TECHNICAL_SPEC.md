...
| `send_test_notification` | `({ source: "football" | "news" | "manual" | "weather" | "agent" }) -> Result<(), String>` (the `"cmux"` variant became `"agent"` with v7's cmux retirement) | enqueues a canned test event with a timestamped body through the same path `/notify` uses; bypasses pause only when the visible slot is empty, otherwise queues normally. priority and TTL are taken from the running config for the selected source. |
...
| `get_about_info` | `() -> Result<AboutInfo, String>` | the About section's system card. `AboutInfo` (camelCase on the wire, `src-tauri/src/about.rs`): `version`, `bundleId`, `bundleSizeBytes: number \| null` (walked from the `.app` bundle root derived off `current_exe()`; `null` in a dev build), `platform` (`sw_vers -productVersion`, e.g. `"macOS 14.5"`, subprocess shim like `notchtap-detect`), `arch` (`std::env::consts::ARCH`), `processMemoryBytes`/`systemMemoryUsedBytes`/`systemMemoryTotalBytes` (targeted `sysinfo` refresh — memory + current process only, not `System::new_all()`), `diskUsedBytes`/`diskTotalBytes: number \| null` (the `sysinfo::Disks` entry mounted at `/`), `uptimeSecs` (elapsed since an `Instant` captured once at boot, managed state). The frontend polls this every 2s while the section is mounted for the live stats; nothing here is emitted as an event. |
