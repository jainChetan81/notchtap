...
- **Design sources of truth (binding)**: `prototypes/tab-notch-rest-and-morph.html` (proposal 1/2 — rest, hover reveal, icon strip, selection, keyboard model) and `prototypes/tab-notch-panel.html` (proposal 2/2 — the six selected-tab cards), both r3, operator-approved 2026-08-02 after three review rounds. Their own header comments and inline annotations are part of this spec by reference — where this document and a mock's own comment disagree on a *value* (a duration, a hex, a class name), the mock wins; where they disagree on a *decision* already resolved by operator instruction (recorded in the mission brief this spec was written from), this document wins.
