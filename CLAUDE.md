...
pre-scaffold. this repo currently contains only planning docs
(`docs/ARCHITECTURE.md`, `docs/IMPLEMENTATION_PLAN.md`,
`docs/TESTING_STRATEGY.md`, `README.md`) — no tauri project has been
created yet, so there are no build/lint/test commands to run today.
the commands below are what will exist once §1.1 of
`docs/IMPLEMENTATION_PLAN.md` is executed (`npm create
tauri-app@latest`). until then, treat any reference to `src-tauri/`,
`src/App.tsx`, or `notchtap` as planned, not present. once scaffolded,
the tauri/rust/web project lives at repo root alongside `docs/` — the
docs folder isn't part of the app build.
