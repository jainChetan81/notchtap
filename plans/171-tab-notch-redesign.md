...
1. Selection state: which tab (if any) is selected. Persists across hover cycles (spec §7's "remembered silently"), cleared if its source stops being live (spec §2 decision 5).
2. Click detection inside the icon strip's own rect, only while that rect is the current hover target — extends the existing tracking-area-adjacent mechanism `hover.rs` already uses for the hover rect (per spec §10, this is the honest new-rust-work item flagged by the mock's own open question 6; do not assume it falls out of the existing `panel_event!` mouseEntered/mouseMoved/mouseExited wiring for free — mouseDown is a different AppKit callback and needs its own registration).
...
4. Emits the selection-changed event (§0) on transitions only.
