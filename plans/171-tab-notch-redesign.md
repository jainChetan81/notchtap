...
2. **Click detection — OPEN, unresolved, blocks the rest of this slice.** `panel_event!` (from the third-party `tauri-nspanel` crate) only exposes mouseEntered/mouseMoved/mouseExited — confirmed by reading its call sites in `lib.rs`, no mouseDown/click callback exists anywhere in that mechanism. Two candidate approaches, neither verified from this Linux dev environment:
   ...
   **Recommendation**: settle this empirically on real macOS hardware — mount a NonactivatingPanel, `set_ignore_cursor_events(false)`, and check with a plain devtools `console.log` in an `onClick` whether a real click reaches the webview at all — before writing more code toward either answer. This is a fast, binary, real-hardware check; guessing further from a Linux VPS risks a nontrivial amount of unverifiable, possibly-wrong native code.
