...
- **Status**: IN PROGRESS — every slice A–K has landed on `feat/tab-notch-redesign` (PR #13). Remaining: operator manual verification on real hardware of the two native boundaries this environment's TCC permissions block (the NSEvent click monitor firing for the never-key NonactivatingPanel, and the prefix keymap's rapid follow-up register/unregister), plus the visual/PAL-multimodal fidelity pass against both mocks. See each slice's "Landed" note below for the as-built state.
- **Commit**: —
...
**Landed (2026-08-03, on-hardware hand-off, commit `59ac361`)**: item 2 RESOLVED as **mechanism (a)** — `src-tauri/src/click.rs`'s NSEvent LOCAL monitor (`objc2-app-kit` `NSEvent`+`block2` features + a direct `block2` dep). The deciding argument was architectural, not just empirical: (b) alone can NEVER satisfy the receive-only overlay — the frontend has no invoke and no emit capability, so a webview-seen click has no channel back to the rust-owned `TabSelection`; and the shipped board-expand scroll path already proves NSEvents reach this NonactivatingPanel while non-key. The monitor gates on hovered + slot-idle + our `windowNumber`, hit-tests `icon_strip_rects` zipped index-for-index with `tabs::present_tabs`, funnels through the shared `apply_tab_select` (lib.rs), and never swallows the event. Item 3 landed in the SAME commit per this plan's own constraint: `set_ignore_cursor_events(false)` opens exactly while hovered∧slot-idle (window-granular is what the OS gives us; the strip-rect narrowing is the monitor's hit-test), reverts on hover-exit unless the board-expand path owns the window, and the slot-promotion latch reset also restores click-through (the promote-while-hovered gap, closed rather than accepted). Item 4 landed: `tab-selection-changed` per §0's exact shape, transitions-only (`status::emit_tab_selection_if_transitioned`), also driven by the engine status pass's `clear_if_gone`. The §0 presence/liveness wire landed as `StatusState.agent.activeSessions` (ungated live-session count — stored by the board publisher BEFORE `gate_presence`, so `board_show_working=false` can't hide a Working session from the icon) + `news.chargeFraction/chargeCount/isCharged`; one `Arc<tabs::TabWire>` bundle threads engine/board/rss/monitor/prefix. TS mirror + validator synced same commit. `capabilities/default.json` byte-identical (diff-verified). `cargo test` 1038, clippy `-D warnings` clean, vitest 717.
...
**Landed (2026-08-03, on-hardware hand-off, commit `c3224d7`)**: the real wiring. `Config.prefix_shortcut` parses through `Code`'s own names (`KeyX`/`DigitN` fallbacks, fail-open to Space) and registers beside the seven untouched combos; arming registers the eleven bare follow-up grabs (1-5 [ ] enter o p esc — never outside a live armed window) with a generation-guarded cancellable disarm timer (`TabWire.prefix_generation`); one consumed key / timeout / esc / prefix-again releases every grab. Routing per `PrefixAction`'s docs: Select → the shared `apply_tab_select` (identical to a click, including the news-visit clear); [ / ] → `TabWire.viewed_session` wrap-cycling gated on agent-selected, emitting `agent-viewed-session-changed {index}` (the §0 session-cycling shape, now pinned); enter/o → `toggle_manual_expand`; p → `toggle_pause`. Prefix-armed frontend indicator: NOT built (spec §4's delight proposal stays operator-gated). Stale dead_code allows removed (prefix/news_charge/hover icon trio). `cargo test` 1038, clippy clean. Live keyboard verification of the rapid register/unregister still operator-owed (TCC blocks synthetic input from this session — see Slice K's verification note).
...
**Landed (2026-08-03, on-hardware hand-off)**: step 1 — `IconStrip` mounts in the hovered idle branch, driven by `lib/iconPresence.ts` (spec §6's table as data; its one refinement: `media.current` non-null while PAUSED keeps music present-but-dim rather than collapsing, so the strip doesn't jump width when a track pauses mid-hover, honoring §2 decision 4). Selection arrives via `useTabSelection.ts` (listens `tab-selection-changed`, strict closed-set validation, no invoke). `TabBelowBlock.tsx` does the selection→below-block swap; none-selected mounts nothing, so §7's "none" page falls out of the conditional rather than being special-cased. `EqBars` at rest gated on `media.current?.playing`. Steps 2/3 — all gates green (cargo test 1037, clippy `-D warnings` clean, vitest 793, tsc/biome clean), `capabilities/default.json` byte-identical throughout. Step 4 — animation lock-down run: three Block-tier findings fixed (five `infinite` per-icon glow loops were ungated and ticking 24/7 behind `visibility: hidden`; icon press feedback rode the 260ms reveal leg on an overshoot curve instead of the 160ms press clock; the pulled below-block jump-cut instead of cross-fading). Lower-tier findings recorded on the PR rather than silently dropped. Additionally, PAL consensus (gemini-2.5-pro vs gpt-5.2) on the bare-follow-up-key grab was unanimous on one mitigation, now implemented: an unconditional generation-ignoring watchdog + release on `RunEvent::Exit`, and failed RELEASES now log at ERROR and keep the retry flag set (a failed register is benign; a failed unregister leaves a bare key grabbed system-wide).

**Still owed (operator, real hardware)**: the two TCC-blocked verifications named in the Status line above, and the football `secondaryMatches` decision (§3b — left empty, which the crossbar variant degrades correctly for; wiring a genuine second live match needs a `StatusState.football` wire change from single `Option` to a list plus a `poller.rs` change to stop collapsing to "first in-play wins").
