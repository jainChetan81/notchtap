...
| 029 | Pin CI actions to commit SHAs + dependabot for github-actions | P3 | S | — | DONE pending first green CI run on push (operator-owed) |
...
| 040 | Weather source + idle-rail ambient presence (football chip + weather chip/alerts) | P2 | S+M | — | Part A DONE (`9b5bd62`), Part B TODO — review-plan pass 2026-07-20 corrected the Config location and found Part B needs an Engine extension (mirroring the football `live`/`update_live_match` pattern) the original filing predated. `/grill-me` pass 2026-07-20 resolved 8 decisions: provider = Open-Meteo (verified live, keyless, no rate limit, no severe-warnings feed), location = raw lat/lon only (no geocoding), chip = plain text no icons (bg/SVG treatment deferred as a future plan), units = configurable default Celsius, v1 alerts = rain-incoming + temp threshold only (no WMO-derived "severe"), alert re-fire = edge-triggered (needs Snapshot-style state tracking in weather_poller.rs), weather_priority default = Medium, Engine plumbing confirmed as proposed. Still open, deferred to a design-doc session: weather_poll_secs default, rotation_order position, exact threshold numbers — Step 0 gates on these. |
