...
| 079 | Full overlay-card visual revamp — consolidated decision session (supersedes 043, 054, 055, 056, 060) | P2 | L (decision) | 042 (DONE), 063 (ships first, sets the "before" baseline) | TODO — filed 2026-07-20 from a `/grilling` session on plan 063 that expanded into a full redesign conversation once the operator shared a reference image. Rather than re-litigate "how should the overlay look" separately across 5 scattered spikes, this is the one file where every overlay visual-language decision gets made in one sitting: card shape/positioning, idle typography, team identification (badges/flags/crests), live-match wire shape, score prominence, event icons (043's confirmed ESPN fallback-chain research folded in as item 6a, ready to execute independently of the display decisions), news/cmux card presentation, weather background art, bottom-row pills, in-card pause discoverability, app icon/branding, asset licensing, and staleness indicators — 14 topics total. Plan 059 (notification history) and 077 (Settings-window log panel) deliberately left out — different kind of decision / different surface, respectively. Plan 065 (BrightData token) flagged as unrelated and more urgent. |
