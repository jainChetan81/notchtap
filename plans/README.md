...
| 077 | Direction: read-only "recent log lines" panel in the Settings window | P2 | S | soft: 070 (more worth reading if it lands first) | TODO — `docs/ARCHITECTURE.md` §11 explicitly promises "when something breaks, the user needs a log to read... set this up in v1" — the write side (plan 014) is built and tested; nothing reads it back anywhere. Grounded direction finding. **Review-plan pass (2026-07-20)**: own read + required fresh-context cold-read (authored in-session) found plan 076 already landed, adding an 8th settings command — the plan's own top-level drift check would have made a literal executor STOP on an already-anticipated, harmless delta; fixed the note to defer to the (already-correct) bottom STOP condition, refreshed "Current state" to the real post-076 8-command baseline, and firmed up the Diagnostics-vs-Connectors section placement recommendation. **Second pass (same day)**: re-verified after further concurrent landings — `logging.rs` still untouched, 8-command baseline and 8 sidebar sections unchanged; found and fixed the drift-check command itself never actually listed `lib.rs`, despite the adjacent prose already naming it as an affected file. File renamed to add `(reviewed)`. |
| 078 | Replace `motion` with CSS transitions in the overlay path; memoize inline markdown (supersedes 069) | P2 | M | none (supersedes 069) | TODO — filed from an operator follow-up question about React Compiler adoption. Investigation found the compiler itself low-urgency (only one unmemoized site in the whole audit), but a live `vite build` turned up a bigger, unflagged issue: the overlay's main chunk is 332 KB/106 KB gzip, 22 `motion` hits in the built chunk, and every `motion`/`AnimatePresence` call site in the overlay path is a simple opacity/y fade or height transition — no drag/gesture/layout-animation usage anywhere. Same category call this repo already made twice (plans 018, 023). Settings window's own `motion` usage is explicitly out of scope. **Review-plan pass (2026-07-20)**: own read + required fresh-context cold-read (authored in-session) found a real structural contradiction — Step 3 claimed `Track`/`Stamp` "keep reading live slot" while both are physically nested inside the exact block being converted to `renderedSlot`. Resolved: everything inside the former `motion.div` (including Track/Stamp) now uniformly reads `renderedSlot`, which doesn't regress expand/collapse (same-key updates already pass through live via the hook's own design). Step 3 rewritten with the full real JSX inlined rather than elided. **Second pass (same day)**: zero drift on all 4 cited files; re-ran `vite build` fresh and got the byte-identical bundle numbers (same content hash) — the plan's core premise re-confirmed, not stale. Fixed Step 2's fake-timers guidance (pointed at two files that don't actually have the pattern — repo-wide search found zero usage anywhere, genuinely new territory) and firmed up a CSS-specificity hedge into a confirmation after working through it by hand. File renamed to add `(reviewed)`. **Third pass (same day)**: zero drift again; extended the check to every child component Step 3's JSX calls into (`IdleView`/`Stamp`/`Track`/`Manifest`, none previously drift-checked directly) — all four confirmed exact prop-signature matches, no new issues. |
