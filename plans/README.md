...
| 039 | ESPN live-match scoreboard card (opt-in, single-match) — Topic `espn:{league}:{match_id}`, `Recurring` rotation, full-time `OneShot` retires it | P2 | M | 037 (DONE), 038 (hard, TODO) | BLOCKED (gated on 038 only — 037 landed `6b53c32`). Review-plan pass 2026-07-19 re-grounded the plan against the landed Engine: `Engine::accept` is confirmed Topic/Rotation-agnostic (passes straight to `queue.enqueue`, which already owns all Topic-supersession/`Recurring` logic — no Engine change needed) and confirmed to preserve connector-fan-out-survives-supersession (clones the event before enqueue, offers unconditionally after). Corrected the Config-surface section (struct is in `config.rs`, not `settings.rs`) and added the frontend TS `Config` interface + `SettingsApp.test.tsx` fixture sites the original filing missed. Wrote concrete numbered Steps (none existed before). Design-complete, code-grounded, ready to execute the moment 038 lands. |
...
| 041 | ESPN event-card copy — label scoring plays with ESPN's own event-type text (goal/penalty/own-goal), matching how cards already self-describe | P3 | S | none (037 landed; 039 coordination is same-file-touch only) | TODO — filed 2026-07-19 from operator feedback watching ENG–FRA live. Review-plan pass 2026-07-19: found the fix is narrower than the original filing assumed — cards already self-describe (`last_card` already prefixes with ESPN's own `detail_type.text`); only the scoring-play path (`last_scoring_play`) discards that same field, and goals are `EventType::ScoreUpdate`, not `MatchState` as originally scoped. Fixture check found real "Goal"/"Penalty - Scored" text in the UCL fixture (no synthesis needed) but zero fixture coverage for own-goal (synthesize per the existing `red_card_emits_red_card_signal` precedent). Recommended dropping the original filing's emoji prefixes (zero precedent anywhere in the codebase) in favor of matching the existing card convention exactly, flagged as a confirm-with-operator choice rather than assumed. Wrote concrete Steps + a shared-helper approach so cards and goals can't drift again. |
