...
| 036 | Heartbeat lost-wakeup: register the `Notified` waiter under the queue lock (renumbered from 024 — that number stays with the in-flight proptest plan) | P1 | S | — | DONE (`9373b84` on `exec/036-heartbeat-lost-wakeup`; `/improve execute` → reviewed APPROVE; cargo 258+3 doc-tests, clippy/fmt clean; `enable()` registered inside queue-lock block) |
