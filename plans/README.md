...
| 063 | SPIKE: idle status rail (460px) overlaps other apps' menu bar icons in notch mode | P1 | S (decision) | — | TODO — **decisions locked 2026-07-20** via `/grilling`, ready for a build plan. Fix is narrowly scoped (no visual restyle): notch-mode width caps to exactly `CutoutGeometry.width` (zero margin), `.src-rail` wraps onto additional rows instead of clipping, 270px serves as both the no-geometry fallback and an absolute floor, the live-match chip ellipsis-truncates if it alone is too wide, and this plan builds the shared `window.__NOTCHTAP_MODE__`/`__NOTCHTAP_CUTOUT_WIDTH__` plumbing that plan 060 will also consume. Full decision record in the plan file's "Grilling session resolved" section. The richer visual-redesign direction the same conversation surfaced (bold typography, team badges, structured match data) was explicitly split out to plan 079, not folded in here. Still needs a real MacBook smoke-check before done. |
...
| 080 | News card implementation: compact published-time meta + full-width expanded summary (079 item 7) | P1 | M | 063 (ships first) | TODO — implements the operator-approved mockup at `prototype/news-card.html`: the compact news card keeps the shipped render (masthead/WIRE stamp/category+age pills/news-shade/track) and gains `published HH:MM` meta at the right end of the pills row (data already on the wire via `publishedAtMs`, formatted by `presentation.ts`'s existing `publishedLabel`); the expanded news manifest drops the 3-column `.manifest-inner.news` grid for a full-width summary + one inline meta row (source · published · category) + footer hints. Files: `StatusRailCard.tsx`/`Manifest.tsx` news branches, `styles.css` + `preview-overlay.css` mirror (same commit — mirror law), minimal vitest updates. First buildable slice of the locked 079 redesign. |
