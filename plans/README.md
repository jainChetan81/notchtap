...
| 042 | Live-match scorecard presentation (richer collapsed card, Option B) | P2 | S–M | 039 (DONE), 037 (DONE) | TODO — **UNBLOCKED, ready to execute.** Review-plan pass 1 (2026-07-20) grounded each option's real cost: `EventMeta.details`/`subtitle` (plan 035) already carry label/value cells with zero schema changes (de-risks Option B) but render ONLY when expanded (`Manifest.tsx:38`); the overlay window has no dynamic-resize path anywhere (Option A's "window grows" is genuinely new capability); per-side yellow/red card counts need new team-id parsing common to every option. Review-plan pass 2 (after 039 landed): a fresh-context cold-read fixed stale "039 not yet executed" framing, repointed citations drifted 5-19 lines, and surfaced that `make_event` is already at clippy's 7-arg ceiling (needs `CardTopic`-style bundling for the new cells, not a param add); corrected a subagent mis-grep claiming `Engine::apply`/`accept` don't exist (they do). `/grill-me` (2026-07-20) resolved the Option A/B/C decision: **Option B — never pin**, the scorecard stays a normal rotating card (existing priority system already keeps football well-represented in rotation; no window resize, no Engine pin mechanism needed). Plus three shape decisions: no flags/icons (reuse existing team abbreviations, zero emoji precedent in this codebase), cards split per side (`ARS 1Y · PSG 0Y`, not a bare total — worth the small `team.id` parsing cost), minimal layout tweak now (Clock + per-side Cards as two new compact lines; a full scoreboard redesign is explicitly deferred to a later plan, after the operator watches the minimal version live). Scope rewritten around Option B only; concrete numbered Steps added (none existed before). |
