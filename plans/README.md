...
| 045 | Bump the stale `tauri-nspanel` git pin (13mo/39 commits behind, missing crash/segfault fixes) | P2 | S–M | — | TODO — review-plan pass (2026-07-20) verified the target rev + commit count against a live upstream fetch (both exact, no drift) and corrected two gaps: a missed second call site (`lib.rs:178`'s `init()`, confirmed unaffected) and upgraded "may hit a compile break" to a confirmed, concrete one — `to_panel()`/`set_style_mask()` both changed signature, with the exact before/after and a working fix pattern (from the crate's own examples) now inlined in the plan. |
| 046 | Docs truth pass: CLAUDE.md/AGENTS.md project-state through plan 042, stale V3_6 spec citation, README source-count/justfile gaps, TESTING_STRATEGY count fix, IMPLEMENTATION_PLAN §6 checklist rows | P2 | S | — | TODO — review-plan pass (2026-07-20) verified all six sub-findings against live content (running `cargo test --locked` where a count was involved, not just grep). Five confirmed accurate. One is a false positive that must NOT be executed as originally written: the "notifier test count is stale, should be 22" claim is wrong — `cargo test --locked notifier:: -- --list` authoritatively reports 23, matching the doc; the plan's own suggested `grep -c` verification undercounts because it misses `notifier.rs:601`'s parameterized `#[tokio::test(flavor = "current_thread")]` attribute. The whole §0 row is in fact already byte-accurate at current HEAD (327 total, matching a live full-suite run). Step 4 rewritten to verify-only via `cargo test --locked <module>:: -- --list` (immune to attribute-syntax variants) rather than apply a wrong fix. |
| 047 | Test-only backfill: poller.rs card team-id-mismatch case, weather_poller.rs threshold-boundary cases, real-timer-test accepted-finding note fix | P2 | S | — (044 has now landed, `5c1ca36`, cleanly — no coordination needed) | TODO — review-plan pass (2026-07-20) hand-traced all four proposed tests against live code (struct visibility, the bucketing loop's None-arm no-op, `diff_weather`'s full threshold logic, exact title strings) and confirmed each compiles and passes exactly as written. Caught plan 044 landing mid-review, which shifted two of this plan's own citations (`ucl_fixture_cards_bucket_per_side_and_color` 1100→1123, the two `engine.rs` real-timer tests 496,536→501,541 via plan 048's unrelated refactor) — both corrected in the plan file. Step 3's README target text confirmed still stale as described. |
