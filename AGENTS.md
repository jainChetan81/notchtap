...
<!-- trimmed 2026-07-22: a plan-by-plan changelog (v1–v6, plans
037–087) lived here, drifting behind the equivalent CLAUDE.md section
(trimmed 2026-07-21). both were reconstructible from plans/done/ and
git log, so this one was cut the same way to keep the two files in
lockstep. -->
...
- **v1 has no approve/deny action.** notifications are display-only,
  ...
  cli" loop without reading `docs/ARCHITECTURE.md` §7 first — that
...
window by default — the settings window's eleven invoke commands
(`clear_history`, `get_config`, `get_connector_health`,
`get_default_config`, `get_history`, `get_recent_log_lines`,
`get_secret_status`, `save_config_and_relaunch`, `set_secret`,
`send_test_notification`, `set_appearance`) are scoped to it alone
only because `src-tauri/build.rs`
